...
import com.frico.easy_pay.utils.PhotoInfo;
...
//            position[1] -= view.getScrollY();
    ...
    public void animaFrom(PhotoInfo photoInfo) {
        if (isInit) {
            ...
            if (photoInfo.mWidgetRect.width() < photoInfo.mImgRect.width() || photoInfo.mWidgetRect.height() < photoInfo.mImgRect.height()) {
                ...
                float clipY = photoInfo.mWidgetRect.height() / photoInfo.mImgRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                ClipCalculate c = photoInfo.mScaleType == ScaleType.FIT_START ? new START() : photoInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
    ...
    public void animaTo(PhotoInfo photoInfo, Runnable completeCallBack) {
        if (isInit) {
            ...
            mTranslate.withRotate((int) mDegrees, (int) photoInfo.mDegrees, mAnimaDuring * 2 / 3);
            ...
            if (photoInfo.mWidgetRect.width() < photoInfo.mRect.width() || photoInfo.mWidgetRect.height() < photoInfo.mRect.height()) {
                ...
                final ClipCalculate c = photoInfo.mScaleType == ScaleType.FIT_START ? new START() : photoInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
