 ...
 * @see UiConfig
 */
public class TagViewMultiGroup extends ViewGroup implements CompoundButton.OnCheckedChangeListener {
    //存储所有的标签
    ...
    public TagViewMultiGroup(Context context) {
        this(context, null);
    }

    public TagViewMultiGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (isInEditMode()) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //子View占据的宽度
            int childWidth = child.getMeasuredWidth() + lp.leftMargin + lp.rightMargin + child.getPaddingLeft();
            //子View占据的高度
    ...
    private void refreshView() {
        ...
        LogUtils.e("小的item===" + Arrays.deepToString(data.toArray()));
        Context context = getContext();
        setPadding(dip2px(context, config.containerPadding), 0, dip2px(context, config.containerPadding), config.containerPadding);
        for (int i = 0; i < data.size(); i++) {
            ...
            button.setTextColor(config.buttonTextColor);
            button.setTextSize(TypedValue.COMPLEX_UNIT_DIP, config.buttonTextSize);
            addView(button);
        }
    }
    ...
    public static class UiConfig {
         ...
         * 整个商品属性的左右间距
         */
        private int containerPadding = 10;
        /**
         * 属性按钮的高度
         */
         ...
         * 属性按钮文字颜色
         */
        private int buttonTextColor = 0xFF666666;
        /**
         * 选择的属性按钮文字颜色
         */
