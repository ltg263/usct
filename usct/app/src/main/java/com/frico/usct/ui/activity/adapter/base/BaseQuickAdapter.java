...
import com.frico.usct.animation.AlphaInAnimation;
import com.frico.usct.animation.LoadMoreView;
import com.frico.usct.animation.ScaleInAnimation;
import com.frico.usct.animation.SimpleLoadMoreView;
import com.frico.usct.animation.SlideInBottomAnimation;
import com.frico.usct.animation.SlideInLeftAnimation;
import com.frico.usct.animation.SlideInRightAnimation;
import com.frico.usct.impl.BaseAnimation;
import com.frico.usct.impl.IExpandable;
...
public abstract class BaseQuickAdapter<T, K extends BaseQuickViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void setEnableLoadMore(boolean enable) {
        ...
        mNextLoadEnable = enable;  //2018.06.28 新增
    ...
    public K onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case LOADING_VIEW:
                ...
                break;
            case HEADER_VIEW:
                baseViewHolder = createBaseQuickViewHolder(mHeaderLayout);
                break;
            case EMPTY_VIEW:
                baseViewHolder = createBaseQuickViewHolder(mEmptyLayout);
                break;
            case FOOTER_VIEW:
                baseViewHolder = createBaseQuickViewHolder(mFooterLayout);
                break;
            default:
    ...
    private K getLoadingView(ViewGroup parent) {
        ...
        K holder = createBaseQuickViewHolder(view);
    ...
    /**
     * 动态设置SpanSize的值，控制样式： 设置item条目的占比  2/6就是一个条目占据两个位置即一排三个， 3/6：一排两个   6/6 一排一个
     * @param recyclerView
     */
    @Override
    ...
    private void bindViewClickListener(final BaseQuickViewHolder baseViewHolder) {
    ...
    protected K onCreateDefViewHolder(ViewGroup parent, int viewType) {
        return createBaseQuickViewHolder(parent, mLayoutResId);
    }

    protected K createBaseQuickViewHolder(ViewGroup parent, int layoutResId) {
        return createBaseQuickViewHolder(getItemView(layoutResId, parent));
    }

    /**
     * if you want to use subclass of BaseQuickViewHolder in the adapter,
     * you must override the method to create new ViewHolder.
     *
     * @param view view
     * @return new ViewHolder
     */
    protected K createBaseQuickViewHolder(View view) {
        Class temp = getClass();
        Class z = null;
        ...
        return null != k ? k : (K) new BaseQuickViewHolder(view);
    }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (BaseQuickViewHolder.class.isAssignableFrom(tempClass)) {
                        return tempClass;
                    }
                }
            }
    ...
    public int setFooterView(View footer) {
        return setFooterView(footer, 0, LinearLayout.VERTICAL);
    }

    public int setFooterView(View footer, int index) {
        return setFooterView(footer, index, LinearLayout.VERTICAL);
    }

    public int setFooterView(View footer, int index, int orientation) {
        if (mFooterLayout == null || mFooterLayout.getChildCount() <= index) {
            return addFooterView(footer, index, orientation);
        } else {
    ...
    public void setEmptyView(View emptyView) {
        ...
        if (mEmptyLayout == null) {
            mEmptyLayout = new LinearLayout(emptyView.getContext());
    ...
    /**
     * set anim to start when mb_loading
     *
     * @param anim
     * @param index
     */
    ...
    /**
     * To open the animation when mb_loading
     */
     ...
     *
     * @param firstOnly true just show anim when first mb_loading false show anim when load the data every time
     */
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, int viewId) {
        ...
        BaseQuickViewHolder viewHolder = (BaseQuickViewHolder) recyclerView.findViewHolderForLayoutPosition(position);
