...
public class StatusTextColorUtils {
     ...
     * @param withoutUseStatusBarColor 是否不需要使用状态栏为暗色调(只有目标版本大于23时才有效)
     */
    public static void setStatusBar(Activity activity, boolean useThemestatusBarColor, boolean withoutUseStatusBarColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0及以上
            ...
            if (useThemestatusBarColor) {
                activity.getWindow().setStatusBarColor(activity.getResources().getColor(R.color.color_eca0a5));
            } else {
        ...
        /**
         * (只有目标版本大于23时才有效)  所以不熟设置就回有效（坑死老子了）
         * app：build.gradle targetSdkVersion 23,24......(都可以)
         */
    ...
    /**
     * 设置状态栏文字色值为深色调
     * @param useDart 是否使用深色调(true 深色调)
     * @param activity
     */
    public static void setStatusTextColor(boolean useDart, Activity activity) {
        if (isFlyme()) {
            ...
            activity.getWindow().getDecorView().findViewById(android.R.id.content).setPadding(0, 0, 0, StatusTextColorUtils.navigationHeight);
        }
    }
}
